...
def decode(chatbot):
    # Create chatbot and load parameters.
    #chatbot = create_chatbot(sess, forward_only=True)
    chatbot.batch_size = 1  # We decode one sentence at a time.

    # Load vocabularies.
    en_vocab_path = os.path.join(chatbot.config_dirs.data_dir, "vocab%d.from" % chatbot.config_dirs.vocab_size)
    fr_vocab_path = os.path.join(chatbot.config_dirs.data_dir, "vocab%d.to" % chatbot.config_dirs.vocab_size)
    ...
    while sentence:
        ...
        _, _, output_logits = chatbot.step(chatbot.sess, encoder_inputs, decoder_inputs,
